...
public class SkipList<K extends Comparable<? super K>, V>
    ...
    /**
     * returns a random level (using geometric distribution), minimum of 1 keep
     * this
     * method private. Since, we do not have any methods to call this method at
     * this
     * time, we keep this publicly accessible and testable.
     *
     ...
     * @return ArrayList<KVPair < K, V>> KVPair sol to search query
     */
    ...
    public void adjustHead(int newLevel) {
        ...
        if (temp.level + 1 >= 0)
            System.arraycopy(temp.forward, 0, head.forward, 0, temp.level + 1);
        head.level = newLevel;
    }
     ...
     * data
     * structure) and null if the pair was not found or invalid.
     *
     * @param key
    ...
    public void dump() {
        ...
        while (current != null) {
            System.out.println("Node has depth " + (current.level) + ", Value ("
                + current.element().getValue().toString() + ")");
            current = current.forward[0];
        }
